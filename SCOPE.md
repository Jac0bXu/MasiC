...
### Phase 3 — Cell library ✅
- [x] `cell_library.py` loader + manifest schema (truth-table completeness, port-inside-footprint, no-duplicate-coord checks).
- [x] Five cells built in Minecraft 1.20.1 + Litematica and saved under `cells/{NOT,NAND2,AND2,OR2,DFF}/cell.litematic`. Footprints + port coordinates measured by walking each schematic with litemapy and normalizing to a (0,0,0) origin.
- [x] Hand-verified in-world: combinational truth tables and DFF rising-edge capture all match the manifests.
...
- [x] **Topological cell ordering + multi-lane router with per-load drop offsets** — the placer topo-sorts cells (drivers before loads, signals flow +x); module-input nets with a single load align directly with the load's z (no separate lane), and every other net gets a unique z-lane with each load dropping at a unique x-offset to avoid same-column drop bridging. `and2` routes cleanly with zero collisions; `full_adder` collides under this router.
- [x] **3D maze router (`router3d.py`)** — sparse voxel grid, redstone-aware A* (horizontal + diagonal-staircase moves), greedy Steiner-tree extension for fan-out, per-net dust-ownership with same-y adjacency exclusion, ascending- and descending-stack prohibitions, cell-perimeter halo, repeater insertion every 14 blocks. CLI flag `--router 3d`. `and2` routes cleanly and verifies in-world; **`full_adder` does not** — either A* exhausts its search when the halo is enforced, or output lamps are driven by spurious power leaking from cell-internal wall-torches into adjacent routing dust when the halo is loose.
- [ ] **Open: declared power-emitter positions per cell.** The router currently treats the entire cell perimeter as risky and forbids the 1-block halo around every cell at port y. That's too broad — most cell-perimeter positions are safe (oak_planks, dirt). Adding a `power_emitters: [(x, y, z), ...]` list to each cell manifest would let the router forbid only the specific halo coords adjacent to wall-torches/comparator outputs/etc., not the whole perimeter. That should make `full_adder` routable in the existing 3D router.
